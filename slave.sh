...
mysql -uroot -p$DBPASSWD -e "STOP SLAVE;"
mysql -uroot -p$DBPASSWD -e "CHANGE MASTER TO MASTER_HOST = '$MASTERHOST', MASTER_USER = '$RUSER', MASTER_PASSWORD = '$RPASSWD', MASTER_LOG_FILE = '$MASTER_LOG_FILE', MASTER_LOG_POS = $MASTER_LOG_POS;"
mysql -uroot -p$DBPASSWD -e "START SLAVE;"
...
mysql -uroot -p$DBPASSWD -h$MASTERHOST -e "USE $DBNAME; CREATE TABLE replica (id int, name varchar(20), surname varchar(20));"
mysql -uroot -p$DBPASSWD -h$MASTERHOST -e "USE $DBNAME; INSERT INTO replica VALUES (1, 'Anatoly', 'Postny');"
mysql -uroot -p$DBPASSWD -h$MASTERHOST -e "USE $DBNAME; INSERT INTO replica VALUES (2, 'Victor', 'Sussky');"
